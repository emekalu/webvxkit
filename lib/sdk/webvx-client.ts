...
/**
 * Configuration options for the WebVX client
 */
export interface WebVXClientConfig {
  /** Your API key */
  apiKey: string;
...
export class WebVXClient {
  ...
  /**
   * Create a new WebVX client
   */
  constructor(config: WebVXClientConfig) {
    this.apiKey = config.apiKey;
    this.apiBaseUrl = config.apiBaseUrl || 'https://your-domain.com/api/external/v1';
    this.debug = config.debug || false;
    
    if (this.debug) {
      console.log('WebVX client initialized with API base URL:', this.apiBaseUrl);
    }
  }
  ...
  public cleanup(): void {
    if (this.debug) {
      console.log('Cleaning up WebVX client');
    }
   ...
   */
  private updateAudioStatus(status: AudioStatus): void {
